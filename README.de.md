...
2. Die Befehlszeile kann den Befehl einfach in der Fernbedienung ausführen und das Ergebnis in Echtzeit zurückgeben, ähnlich wie SSH
3. Wenn Sie den Service -Host automatisch finden, können Sie auch den Verbindungshost manuell angeben
4. Der Vergleich von Ordnern kann Informationen der Dateien in den beiden Ordnern, dasselbe, Unterschiede usw. anzeigen.
...
2. Es kann in jeder Netzwerkumgebung wie dem örtlichen Netzwerk und dem öffentlichen Netzwerk verwendet werden.
3. Multi -Thread -Getriebe, schnelle Übertragungsgeschwindigkeit kann in der tatsächlichen Messung über 1000 Mbit / s die Bandbreite ausführen. Aufgrund der Ausrüstungsgrenze wird keine höhere Bandbreite getestet
4. Der Speicherberuf ist während der Laufzeit klein
5. Das heißt, offen und ausschalten, bleibt der Prozess nicht

### wie man wählt

1. Wenn Sie einen leistungsstärkeren Dateiübertragungsdienst wünschen, wählen Sie FTP Server und Client (z. `FileZilla` Ebenso gut wie `WinSCP` Warten)
...
## Installation und Betrieb
 ...
 `FTS` Occupy Port 2023.2021, FTC nimmt Port 2022 ein. Unter ihnen wird Port 2023 als verwendet als `FTS` Der TCP -Hörport 2021, 2022 als UDP -Übertragungsschnittstelle zwischen dem Server und dem Client. Sie können die Details am Ende dieses Artikels überprüfen.

### Laden Sie das ausführbare Programm herunter

1. Klicken Sie auf rechts `Release` 
2. herunterladen `File Transfer Tools.zip` 
3. Unzippordner, Double -klick `FTC.exe`  oder  `FTS.exe`  Lauf einfach
...
2. verwenden `pip install -r requirements.txt` Installieren Sie alle Abhängigkeiten
...
#### Parameterbeschreibung
 ...
 `-host` : Geben Sie explizit den empfangenden Seitenhost an (unter Verwendung von Hostname oder IP -Adresse). Wenn diese Option nicht verwendet wird, findet der Client automatisch **Das gleiche Subnetz** Der Kellner

 `-p` : Kooperieren `-host`  Verwenden Sie, da die beiden Parteien automatisch Informationen austauschen und so im Allgemeinen nicht angegeben werden müssen. Nur wenn die beiden Parteien normalerweise nicht eine Verbindung herstellen können, muss sie anscheinend angegeben werden.
...
3. eingeben `speedtest n` Testen Sie dann die Geschwindigkeit des Netzwerks. **Netz** In der Mitte 1 GB = 1000 MB = 1000000 kb.
4. eingeben `compare local_dir dest_dir` Vergleichen wir den Unterschied zwischen dem Ordner und dem Serverordner.
5. Führen Sie bei der Eingabe anderer Inhalte als Anweisung durch und geben Sie die Ergebnisse in Echtzeit zurück.
...
#### Parameterbeschreibung
 ...
 `-d, --dest` : Geben Sie die Speicherposition der Datei an, wenn sie nicht angegeben ist, wird sie auf den aktuellen Benutzer gespeichert. **Desktop** Dann dann
