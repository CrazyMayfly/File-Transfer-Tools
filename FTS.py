...
class FTS:
    def __init__(self, ftt):
        self.__ftt = ftt
        self.main_conn: ESocket = ftt.main_conn_recv
    ...
    def __compare_folder(self, folder):
        ...
        if not os.path.exists(folder):
            # 发送目录不存在
            self.main_conn.send_size(CONTROL.CANCEL)
            return
        self.main_conn.send_size(CONTROL.CONTINUE)
        # 将数组拼接成字符串发送到客户端
        self.main_conn.send_with_compress(get_files_info_relative_to_basedir(folder))
        if self.main_conn.recv_size() != CONTROL.CONTINUE:
            return
        self.logger.log("Continue to compare hash")
        file_size_and_name_both_equal = self.main_conn.recv_with_decompress()
        # 得到文件相对路径名: hash值字典
        results = {filename: get_file_md5(PurePath(folder, filename)) for filename in
                   file_size_and_name_both_equal}
        self.main_conn.send_with_compress(results)
    ...
    def __execute_command(self, command):
        ...
        while result := out.readline():
            self.main_conn.send_head(result, COMMAND.EXECUTE_RESULT, 0)
            output.append(result)
        # 命令执行结束
        self.main_conn.send_head('', COMMAND.FINISH, 0)
